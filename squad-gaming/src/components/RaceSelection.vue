...
export default {
  ...
  computed: {
    imageSrc() {
      return this.result?.image ? new URL(`${import.meta.env.BASE_URL}races/${this.result?.image}`, import.meta.url).href : "";
    },
    shareLink() {
      return this.result?.text ? `whatsapp://send?text=Mein Volk ist: ${this.result.text}!!` : ""
    }
  },
  methods: {
    ...
    loadXMlFile() {
      ...
      xhttp.open("GET", `${import.meta.env.BASE_URL}surveys/race-survey.de.xml`);
